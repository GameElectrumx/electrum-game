...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-GAME.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-game-macos
	cd /tmp/electrum-game-macos
...
rm -rf /tmp/electrum-game-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-game-macos/image/
cp -r $1 /tmp/electrum-game-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-GAME_uncompressed.dmg \
    /tmp/electrum-game-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-GAME_uncompressed.dmg electrum-game-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-GAME_uncompressed.dmg

echo "Done."
md5sum electrum-game-$VERSION.dmg
