...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-GAME"
  !define PRODUCT_WEB_SITE "https://github.com/samadsajanlal/electrum-game"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-game-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-game\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-game\*.*"
  File "c:\electrum-game\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-game-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-game-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-game-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-game-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-game-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\gamecredits" "" "URL:gamecredits Protocol"
  WriteRegStr HKCU "Software\Classes\gamecredits" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\gamecredits" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\gamecredits\shell\open\command" "" "$\"$INSTDIR\electrum-game-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\gamecredits"
