...
version=`python3 -c "import electrum_game; print(electrum_game.version.ELECTRUM_VERSION)"`
...
for mine in $(ls dist/*.exe); do
    echo "---------------"
    f=$(basename $mine)
    echo "Downloading https://electrum-game.org/download/$f"
    wget -q https://electrum-game.org/download/$f -O signed/$f
